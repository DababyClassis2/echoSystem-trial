...
class StorageService {
  ...
  String get deviceName {
    ...
    return 'EchoDevice';  // sync fallback; real default can be set later via async method
  }
  ...
  String get defaultSavePath {
    ...
    // default to Downloads/echoSystem (Android only for now)
  ...
  String _getDownloadsPath() {
    // Android only – use a fixed path that exists on most devices
    if (Platform.isAndroid) {
      return '/storage/emulated/0/Download';
    } else if (Platform.isIOS) {
      // Return a sync placeholder; actual iOS path can be obtained async when needed
      return '/';
    }
  ...
  // --- Transfer History ---
  List<TransferModel> get transferHistory => _transferBox.values.toList();
  ...
  Future<void> addTransfer(TransferModel transfer) async {
    ...
    if (_transferBox.length > 500) {
      ...
      for (var key in oldestKeys) await _transferBox.delete(key);
    }
  }
...
class DeviceModelAdapter extends TypeAdapter<DeviceModel> {
  ...
  final int typeId = 0;
  @override
  DeviceModel read(BinaryReader reader) => DeviceModel(
...
class TransferModelAdapter extends TypeAdapter<TransferModel> {
  ...
  final int typeId = 1;
  @override
  TransferModel read(BinaryReader reader) => TransferModel(
        id: reader.readString(),
